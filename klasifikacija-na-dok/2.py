...
if __name__ == '__main__':
    recenica = input()
    dolzina04 = int(len(train_data) * 0.4)
    dolzina08 = dolzina04 * 2
    prvtrain = train_data[:dolzina04]
    vtortrain = train_data[dolzina04:dolzina08]
    testdata = train_data[dolzina08:]
    klasifikator1 = NaiveBayes(get_words)
    klasifikator2 = NaiveBayes(get_words)
    for row in prvtrain:
        klasifikator1.train(row[0], row[1])
    for row in vtortrain:
        klasifikator2.train(row[0], row[1])
    tocnost = 0
    for row in testdata:
        pred1 = klasifikator1.classify_document(row[0])
        pred2 = klasifikator2.classify_document(row[0])
        if pred1 == pred2:
            if pred1 == row[1]:
                tocnost += 1
    print(tocnost/len(testdata))
    predviduvanje1=klasifikator1.classify_document(recenica)
    predviduvanje2=klasifikator2.classify_document(recenica)
    if predviduvanje1 == predviduvanje2:
