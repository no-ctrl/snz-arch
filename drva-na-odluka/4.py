...
if __name__ == '__main__':
    ...
    for x in range(0, len(train_data)):
        if x in test_indices:
            testSet.append(train_data[x])
        else:
            newTrainData.append(train_data[x])

    dolzina = len(newTrainData) // 2
    tree1 = build_tree(newTrainData[:dolzina], entropy)
    tree2 = build_tree(newTrainData[dolzina:], entropy)
    brojac1 = 0
    brojac2 = 0
    for red in testSet:
        predviduvanje1 = classify(red, tree1)
        predviduvanje2 = classify(red, tree2)
        for x in predviduvanje1.keys():
            if x == red[-1]:
                brojac1 += 1
        for x in predviduvanje2.keys():
            if x == red[-1]:
                brojac2 += 1
    tocnost1 = brojac1 / len(testSet)
    tocnost2 = brojac2 / len(testSet)
    print(f"Tochnost na drvoto 1: {tocnost1}")
    print(f"Tochnost na drvoto 2: {tocnost2}")
    
    if (tocnost1 == tocnost2):
        print("Drvata imaat ista tochnost")
    elif tocnost1 > tocnost2:
