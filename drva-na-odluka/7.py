...
if __name__ == '__main__':
    ...
    t1 = build_tree(train1)
    t2 = build_tree(train2)
    t3 = build_tree(train3)
    glasovi = {0: 0, 1: 0, 2: 0}
    pred1 = max(classify(test_case, t1).items(), key=lambda x: x[1])[0]
    pred2 = max(classify(test_case, t2).items(), key=lambda x: x[1])[0]
    pred3 = max(classify(test_case, t3).items(), key=lambda x: x[1])[0]
    glasovi[pred1] += 1
    glasovi[pred2] += 1
    glasovi[pred3] += 1
    klasa=max(glasovi.items(),key=lambda x:x[1])

    print("Glasovi: "+str(glasovi))
    print("Predvidena klasa: "+str(klasa[0]))
