...
if __name__ == "__main__":
    ...
    roaches = [x for x in data if x[-1] == "Roach"][:40]
    pikes = [x for x in data if x[-1] == "Pike"][:40]
    train_set = roaches + pikes

    tree1 = build_tree(train_set)
    klasifikator = max(classify(test_case, tree1), key=lambda x: x[1])
    print(klasifikator)
