...
if __name__ == '__main__':
    ...
    input4=float(input())
    input5=input()
    dolzina=len(trainingData)//2
    tree1=build_tree(trainingData[:dolzina])
    tree2=build_tree(trainingData[dolzina:])

    test_case=[input1,input2,input3,input4,input5]
    klasifikator1=max(classify(test_case,tree1),key=lambda x: x[1])
    klasifikator2 = max(classify(test_case, tree2), key=lambda x: x[1])
   # print(klasifikator1)
    if klasifikator1 == klasifikator2:
        print(klasifikator1)
    else:
        print("KONTRADIKCIJA")