...
def build_tree(rows, scoref=entropy, level=-1):
    ...
    if best_gain > 0:
        true_branch = build_tree(best_sets[0], scoref, level=level)
        false_branch = build_tree(best_sets[1], scoref, level=level)
        return DecisionNode(col=best_criteria[0], value=best_criteria[1],
                            tb=true_branch, fb=false_branch, level=level)
    else:
        return DecisionNode(results=unique_counts(rows), level=level)
...
if __name__ == "__main__":
    ...
    dolzina = int(len(dataset) * 0.8)

    train_data = dataset[:dolzina]
    test_data = dataset[dolzina:]
    ...
    tree2 = build_tree(train_data2)
    # Vashiot kod tuka
    br1 = 0
    br2 = 0
    for el in test_data:
        kl1 = max(classify(el[:-1], tree).items(), key=lambda x: x[1])[0]
        if kl1 == el[-1]:
            br1 += 1
    for el in test_data2:
        kl2 = max(classify(el[:-1], tree2).items(), key=lambda x: x[1])[0]

        if kl2 == el[-1]:
            br2 += 1
    tocnost1 = br1 / len(test_data)
    tocnost2 = br2 / len(test_data2)
    print(f"Tochnost so prvoto drvo na odluka: {tocnost1}")
    print(f"Tochnost so vtoroto drvo na odluka: {tocnost2}")
