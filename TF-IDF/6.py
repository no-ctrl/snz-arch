...
if __name__ == '__main__':
    ...
    keywords_input = []
    vec_input = process_document_with_words(test_doc, df, n, vocab)
    for el in vec_input:
        if el[1] > 0:
            keywords_input.append(el[0])
    recnik = {}
    dok = 0
    zaednicki = []
    for doc in docs:
        keywords = []
        pom = []
        vec = process_document_with_words(doc, df, n, vocab)
        for el in vec:
            if el[1] > 0:
                keywords.append(el[0])
        br = 0
        for word in keywords:
            for word2 in keywords_input:
                if word == word2:
                    br += 1
                    pom.append(word2)
        zaednicki.append(pom)
        recnik[dok] = br
        dok += 1
        
    rez = sorted(recnik.items(), key=lambda x: x[1], reverse=True)[:4]
