...
if __name__ == '__main__':
    text=input()
    docs=[x[0] for x in train_data]
    labels=[x[1] for x in train_data]
    p_pear=rank_documents(text,docs,pearson)
    p_cosin=rank_documents(text,docs,cosine)
    listap=[]
    listac=[]
    for item in p_pear:
        listap.append(item[1])
    for item in p_cosin:
        listac.append(item[1])
    flag=True
    for i in range(0,4):
        print(f"Rang po slicnost:{i} Cosine-Document #{listac[i]} Pearson-Document #{listap[i]}")
        if listap[i] != listac[i]:
            flag=False
